...
from muggle.constants import modules_enabled, enable_modules, SYSTEM
...
from muggle.constants import BLACKLIST, IP_COUNTS
from muggle.config import sys_args
from types import SimpleNamespace
from muggle.pages.utils import BlocksFuse, BlocksEntities

Logic = TypeVar('Logic', bound=BaseLogic)
interface_map = {}

sdk_module = importlib.import_module("muggle.core.sdk")
...
class Handler:
    ...
    def process(
        ...
        if 'Charge' in modules_enabled and api_type == APIType.TEXT:
            ...
            admin = sys_args.get('admin')
            if not token or not token.available(project_name=project_name):
                raise ServerException(
                    message=f"中间件<Charge>过程失败: 当前授权不存在或者已过期, 请联系管理员{f', {admin}' if admin else ''}",
                    code=4053,
                    api_type=api_type,
                    project_name=project_name,
                    request=request
...
if enable_modules('!Draw&!Docs'):
    interface = SimpleNamespace(app=FastAPI())
elif enable_modules('Draw'):
...
if enable_modules('Docs|Draw'):
    interface = BlocksFuse(**interface_map)
    interface.setting_routes()

if enable_modules('Charge'):
    if SYSTEM == 'Windows':
        token_pool = dict()
    else:
        token_pool = get_context('fork').Manager().dict()
    token_pool.update({})
    interface = globals().get('interface', None)
    if not interface and not enable_modules('Docs'):
        raise RuntimeError("模块 [Charge] 必须依赖于模块 [Docs]")
    Import.dynamic_import(
        "muggle.middleware.charge.Charge",
        instance=True, ctx=token_pool, interface=interface
    )
    charge = Import.get_class('Charge')

if enable_modules('MemoryLoader'):
    # interface = getattr(__main__, 'interface', None)
    # print(interface.app)
    # if not interface and not enable_modules('Docs&Draw'):
    #     raise RuntimeError("模块 [Charge] 必须依赖于模块 [Docs, Draw]")
    interface = globals().get('interface', None)
    Import.dynamic_import(
...
if sys_args['warm_up']:
    # memory_loader = globals().get('memory_loader', None)
